...
- [x] **R17: Design V2 Review Sound Feedback (`wrong` and `done`)**
      ...
      4. Verification: `fvm flutter test test/review/review_controller_test.dart` 32/32, including three new tests. Proven non-vacuous — reverting the two hook points fails `a rejected move plays the wrong sound` and `running the queue out plays the done sound exactly once` with `Expected: <1> Actual: <0>`. A third test pins that an empty app stays silent on startup, guarding `isComplete`'s `hasStudies` conjunct; it passes either way by design. *(done: 2026-09-26)*
      5. **Not done, needs owner sign-off:** `design/docs/07` §1 wants all move/capture/UI sounds replaced by this set with `standard/futuristic/lisp/nes/piano/sfx` cut, and §6 wants sound **default off** where `GeneralPrefs.defaults.isSoundEnabled` is `true`. Both are cuts, so per `AGENTS.md` §9 they are not taken unilaterally. As shipped, review `move` still uses the user's chosen theme. Also unchased: the spec wants `move` on the *user's* landing too, and the user's drag does not go through `moveFeedback`. *(open: 2026-09-26)*
