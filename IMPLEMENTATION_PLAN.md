...
- [x] **R17: Design V2 Review Sound Feedback (`wrong` and `done`)**
      1. `assets/sounds/diagram/{move,wrong,done}.wav` shipped in the tree and were declared in `pubspec.yaml`, but nothing loaded or played them. The review flow was silent at the two moments that matter: a move that was not the repertoire's, and a session that ran out. `move` was already wired via `MoveFeedbackService` and was left alone.
      2. Added `ReviewSound` (`move`/`wrong`/`done`) loaded from a fixed `assets/sounds/diagram/*.wav` path, preloaded at startup and reloaded in `changeTheme` (whose `release()` would otherwise leave the set silent after any theme change). Kept out of the `SoundTheme` enum on purpose: the themes resolve by name with a `standard/` fallback and ship `.mp3`/`.aifc`, so folding these in would make every theme switch try to resolve two sounds that exist in no theme. `SoundPool` and `AVAudioPlayer` read `.wav` natively — nothing to transcode.
      3. `wrong` fires on the rejected branch of `onUserMove`; `done` fires on the rising edge of `ReviewScreenState.isComplete` via `listenSelf`, because that is a derived getter and a session can end three ways (queue drained, daily quota, study deactivated) with no single assignment site. The edge matters: the state is re-emitted continuously once a session is finished.
      4. Follow-up, same contract: the user's own piece landing was silent. The spec asks for `move` on both landings; only the opponent's auto-reply was wired, because the board widget plays nothing and `onUserMove` went straight to the state assignment. Wired in `_handleCorrectAdvancement`, playing the theme's `Sound.move` to match the existing call sites — which set should own a landing is the open cut question below, but that the moment makes *any* sound is not. Also fixed an asymmetry this exposed: only the first attempt's rejection was wired, so a still-wrong reguess passed unacknowledged.
      5. Verification: `fvm flutter test test/review/review_controller_test.dart` 34/34. Every new assertion checked non-vacuous by removing the hooks and watching it fail with `Expected: <1> Actual: <0>`. The first version of the user's-landing test was itself vacuous — a `1. d4 d5` line with `count >= 1` passed without the hook, because the opponent's reply also plays `Sound.move`; it now uses a line with no reply so exactly one move sound can only be the user's landing. *(done: 2026-09-26)*
      6. **Not done, needs owner sign-off:** `design/docs/07` §1 wants all move/capture/UI sounds replaced by this set with `standard/futuristic/lisp/nes/piano/sfx` cut, and §6 wants sound **default off** where `GeneralPrefs.defaults.isSoundEnabled` is `true`. Both are cuts, so per `AGENTS.md` §9 they are not taken unilaterally. *(open: 2026-09-26)*
